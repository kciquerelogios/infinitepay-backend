export default async function handler(req, res) {
  ...
  if (secret !== process.env.REPROCESSAR_SECRET) {
    return res.status(401).send(`<html><body style="font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh"><div style="text-align:center"><h2>🔒 Acesso Restrito</h2><form onsubmit="window.location.href='/api/dashboard?secret='+document.getElementById('s').value;return false" style="margin-top:20px"><input id="s" type="password" placeholder="Senha" style="padding:10px;border:1px solid #ddd;border-radius:8px;font-size:15px"><button type="submit" style="padding:10px 20px;background:#2563eb;color:#fff;border:none;border-radius:8px;margin-left:8px;font-size:15px;cursor:pointer">Entrar</button></form></div></body></html>`);
  }
  ...
  const totalValor = leads.reduce((s, l) => s + (l.carrinho || []).reduce((cs, i) => cs + (i.preco * i.quantidade / 100), 0), 0);
  ...
  const rows = leads.map(lead => {
    ...
    return `<tr>
      ...
      <td><span class="badge-estagio" data-estagio="${lead.estagio}" data-ref="${dataRef}">carregando...</span></td>
      ...
      <td style="white-space:nowrap">
        ${telefone ? `<a href="https://wa.me/55${telefone}?text=${msg}" target="_blank" class="btn-wpp"><svg width="14" height="14" viewBox="0 0 24 24" fill="#fff"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.124.553 4.122 1.522 5.862L.057 23.57a.5.5 0 00.614.612l5.807-1.524A11.935 11.935 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.882a9.856 9.856 0 01-5.031-1.378l-.36-.214-3.733.979.997-3.648-.235-.374A9.856 9.856 0 012.118 12C2.118 6.52 6.52 2.118 12 2.118S21.882 6.52 21.882 12 17.48 21.882 12 21.882z"/></svg> WhatsApp</a>` : '<span style="font-size:12px;color:#9ca3af">Sem telefone</span>'}
        <a href="/api/dashboard?secret=${secret}&del=${lead.id}&t=${Date.now()}" class="btn-del" onclick="return confirm('Remover este lead?')"><svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M9 6V4h6v2"/></svg></a>
      </td>
    </tr>`;
  }).join('');
...
.btn-refresh { padding: 8px 16px; background: #f3f4f6; border: 1px solid #e8eaf0; border-radius: 8px; font-size: 13px; text-decoration: none; color: #374151; }
...
.btn-del { display: inline-flex; align-items: center; padding: 8px; background: #fef2f2; color: #dc2626; border: 1px solid #fecaca; border-radius: 8px; margin-left: 6px; text-decoration: none; }
...
@media (max-width: 768px) { .stats { grid-template-columns: 1fr; padding: 16px; } .container { padding: 0 16px 32px; } .header { padding: 16px; } table { font-size: 12px; } th, td { padding: 8px 10px; } }
</style>
</head>
<body>
<div class="header">
  <div><h1>🛒 Carrinhos Abandonados</h1><div class="header-sub">Kcique Relógios</div></div>
...
<div class="stats">
  ...
  <div class="stat-card"><div class="stat-label">Abandonaram no Pagamento</div><div class="stat-value" id="count-abandon">0</div></div>
...
<div class="container">
  ...
  ${leads.length === 0 ? '<div class="vazio">Nenhum carrinho abandonado ainda! 🎉</div>' : `<table><thead><tr><th>Cliente</th><th>Estágio</th><th>Produtos</th><th>Valor</th><th>Data</th><th>Ação</th></tr></thead><tbody>${rows}</tbody></table>`}
</div>
<script>
var badgeMap = {
  'email': '<span class="badge" style="background:#f3f4f6;color:#374151">⚪ Só email</span>',
  'dados': '<span class="badge badge-dados">🟡 Preencheu dados</span>',
  ...
  'frete_calculado': '<span class="badge" style="background:#ede9fe;color:#5b21b6">🟣 Calculou frete</span>',
  'frete_selecionado': '<span class="badge" style="background:#d1fae5;color:#065f46">🟢 Selecionou frete</span>',
...
var agora = new Date();
var countAbandon = 0;
document.querySelectorAll('.badge-estagio').forEach(function(el) {
  var estagio = el.getAttribute('data-estagio');
  var ref = el.getAttribute('data-ref');
  if (estagio === 'pagamento_pendente' && ref) {
    var minutos = (agora - new Date(ref)) / 1000 / 60;
    if (minutos >= 10) { estagio = 'abandonou_pagamento'; }
  }
  if (estagio === 'abandonou_pagamento') countAbandon++;
  el.outerHTML = badgeMap[estagio] || '<span class="badge badge-dados">🟡 ' + estagio + '</span>';
});
document.getElementById('count-abandon').textContent = countAbandon;
</script>
</body>
</html>`);
}
