export default async function handler(req, res) {
  ...
  // Deletar oferta
  if (req.query.del) {
    await fetch(`${KV_URL}/del/${encodeURIComponent(req.query.del)}`, { method: 'POST', headers: { Authorization: `Bearer ${KV_TOKEN}` } });
    ...
    return res.status(200).send(`<html><head><meta http-equiv="refresh" content="0;url=/api/ofertas-dashboard?secret=${secret}"></head><body>Redirecionando...</body></html>`);
  }
  ...
  try {
    ...
    for (const id of ids) {
      try {
        const r = await fetch(`${KV_URL}/get/${encodeURIComponent(id)}`, { headers: { Authorization: `Bearer ${KV_TOKEN}` } });
  ...
  const rows = ofertas.map(o => {
    const dataLocal = new Date(o.dataHora);
    const dataStr = dataLocal.toLocaleDateString('pt-BR') + ' ' + dataLocal.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
    ...
    return `<tr>
      ...
      <td style="padding:12px 16px;border-bottom:1px solid #f3f4f6;white-space:nowrap">
        <a href="/api/ofertas-dashboard?secret=${secret}&del=${o.id}" onclick="return confirm('Remover esta oferta?')" style="display:inline-flex;align-items:center;padding:6px 10px;background:#fef2f2;color:#dc2626;border:1px solid #fecaca;border-radius:6px;text-decoration:none;font-size:12px">🗑 Remover</a>
      </td>
    </tr>`;
  }).join('');

  const gruposCheckboxes = GRUPOS.map(g => `<label style="display:inline-flex;align-items:center;gap:4px;margin:4px;padding:4px 10px;border:1px solid #e8eaf0;border-radius:6px;cursor:pointer;font-size:13px"><input type="checkbox" value="${g.id}" checked> ${g.nome}</label>`).join('');
...
<div class="header">
  ...
  <a href="/api/dashboard?secret=${secret}" style="color:#aaa;font-size:13px;text-decoration:none">← Carrinhos Abandonados</a>
</div>
<div class="container">
  <div class="form-card">
    ...
    <div class="field">
      ...
      <textarea id="f-texto" placeholder="🔥 OFERTA RELÂMPAGO!&#10;&#10;Relógio X por R$ 199,90&#10;&#10;Clique no link e garanta o seu!"></textarea>
    </div>
    <div class="field">
      <label>URL da imagem (opcional)</label>
      <input type="url" id="f-imagem" placeholder="https://cdn.shopify.com/...">
    </div>
    <div class="field">
      <label>Link do produto (opcional)</label>
      <input type="url" id="f-link" placeholder="https://kcique.com.br/products/...">
    </div>
    <div class="field">
...
async function salvarOferta() {
  ...
  if (!texto) { msg.textContent = '⚠️ Digite o texto da mensagem'; msg.style.color = '#ef4444'; return; }
  if (!dataHora) { msg.textContent = '⚠️ Selecione a data e hora'; msg.style.color = '#ef4444'; return; }
