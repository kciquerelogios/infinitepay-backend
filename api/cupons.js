...
function _attachCupons(){
  ...
  ct().addEventListener('change',function(e){
    if(e.target.name==='c-prod-chk'){
      ...
      var sels=Array.from(document.querySelectorAll('input[name="c-prod-chk"]:checked')).map(function(i){return i.value;});
      var btn=document.getElementById('btn-toggle-prods');
      if(btn) btn.textContent=(sels.length?'✅ '+sels.length+' produto(s) selecionado(s) — clique para editar':'▶ Selecionar produtos (clique para expandir)');
    }
